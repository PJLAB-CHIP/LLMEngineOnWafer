...
class Simulator:
    ...
    def convert_time_to_seconds(self, t):
    # 分割时间字符串，例如 '15:46.7' -> ['15', '46.7']
        parts = t.split(':')
        
        # 分别提取分钟和秒部分
        minutes = int(parts[0])  # 将分钟部分转换为整数
        seconds_with_ms = float(parts[1])  # 将秒部分（包括毫秒）转换为浮点数
    ...
    def schedule(self, delay, action):
        ...
        if isinstance(delay, str):
            delay = self.convert_time_to_seconds(t=delay)
        event = Event(self.time + delay, action)
        heapq.heappush(self.events, event)  # 二叉树结构的优先队列
        return event
    ...
    def load_trace(self):
        ...
        for request in self.trace.requests:
            self.schedule(request.arrival_timestamp,
                          lambda request=request: self.router.request_arrival(request))  # lambda arguments: expression
