...
def test_mapper(model, arch, details=True):
    ...
    print(ops)
    tile_m_tile_n_g = [[128, 64], [256, 32], [128, 32],
                       [32, 128], [64, 64], [256, 32], [64, 128]]
    ...
    for tile_m_tile_n in tile_m_tile_n_g:
        ...
        utilization = gemm_tiling_input_stationary(
            B, M, K, N, tile_m, tile_n, print_details=details)
        print(
            f"test_gemm_daixu, M={M}, K={K}, N={N}, B={B}, tile_m={tile_m}, tile_n={tile_n}, stationary: input, utilization={utilization:.2f}%")
        mapping_result['test_gemm'] = gemm_auto_opt_mapper(
            ops['test_gemm'], arch, input_stationary=True, Tm_Tn=Tm_Tn, details=details)
        print('test_gemm,latency={},cp_latency={},utilization={:.2f}%'.format(
            mapping_result['test_gemm']['latency'], mapping_result['test_gemm']['cp_latency'], mapping_result['test_gemm']['utilization']*100))
        print('-'*80)

        utilization = gemm_tiling_weight_stationary(
            B, M, K, N, tile_m, tile_n, print_details=details)
        print(
            f"test_gemm_daixu, M={M}, K={K}, N={N}, B={B}, tile_m={tile_m}, tile_n={tile_n}, stationary: weight, utilization={utilization:.2f}%")
        mapping_result['test_gemm'] = gemm_auto_opt_mapper(
            ops['test_gemm'], arch, input_stationary=False, Tm_Tn=Tm_Tn, details=details)
        print('test_gemm,latency={},cp_latency={},utilization={:.2f}%'.format(
            mapping_result['test_gemm']['latency'], mapping_result['test_gemm']['cp_latency'], mapping_result['test_gemm']['utilization']*100))
    ...
    mapping_result['Flashatten'] = flashatten_mapper(
        model, arch, Tx_Ty=Tx_Ty, details=details, Head_fused=False)
...
if __name__ == "__main__":
    llm_config = load_config("./input/llama2_7b/input0.json")
